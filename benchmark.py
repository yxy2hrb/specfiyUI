...
from baseline_utils import take_screenshot, cleanup_response, encode_image, extract_text_from_html, index_text_from_html
import json
...
def gpt4o_call(openai_client, base64_image, prompt):
	...
	# prompt_tokens, completion_tokens, cost = gpt_cost("", response.usage)
...
@retry.retry(tries=3, delay=2)
def gpt4o_revision_call(openai_client, base64_image_ref, base64_image_pred, prompt):
	url = 'https://dashscope.aliyuncs.com/compatible-mode/v1'
	token = ""
	openai_client = OpenAI(api_key=token, base_url=url)
	response = openai_client.chat.completions.create(
...
def direct_prompting(openai_client, image_file):
	...
	direct_prompt += "If it involves any charts or table, use recharts for those elements.\n"
	...
	## call GPT-4V
	html, prompt_tokens, completion_tokens, cost = gpt4o_call(openai_client, base64_image, direct_prompt)

	return html, prompt_tokens, completion_tokens, cost

def extract_text_from_image(image_file):
	'''
	Extracts all text from the given image file.
	'''

	orc_prompt = """
		你是一名专业的视觉分析助手，请帮助我识别以下UI界面截图中的可视文本内容，并按照组件语义结构进行归类。

		图像内容是一个UI页面（例如：Web应用、后台系统、APP界面等）。

		请你完成以下任务：

		1. **识别所有文本内容**
		- 包括按钮、标签、导航栏文字、表单字段名、卡片标题、提示文本等
		- 忽略图标、图形装饰、无意义的字母或编码（如svg、路径名）
		...
		- 支持的组件类型包括但不限于：
			...
			- 表格列名（Table Header）
			- Tab 名称（Tab）
			- 悬浮提示 / 状态信息（Tooltip / Status）
			- 顶部导航栏（Top Navigation）
			- 页脚 / 分页（Footer / Pagination）
		...
		```json
		{
		"Top Navigation": ["对账管理", "设置", "通知", "用户中心"],
		"Tab": ["待办任务", "我的申请"],
		"Card Title": ["对账计划", "对账单", "已完成", "未完成"],
		"Input Label": ["客户", "签约主体", "区域", "是否超期"],
		"Button": ["转处理", "查询", "重置"],
		"Pagination": ["共500条", "前一页", "后一页", "跳转"]
		}
"""
	# url = 'https://dashscope.aliyuncs.com/compatible-mode/v1'
	# token = ""
	# client = OpenAI(api_key=token, base_url=url)
	text = gpt_infer(image_file, orc_prompt, model=model_vision)
	return text
...
def text_augmented_prompting(openai_client, image_file):
	...
	texts = "\n".join(extract_text_from_image(image_file))
	...
	## call GPT-4V
	html, prompt_tokens, completion_tokens, cost = gpt4o_call(openai_client, base64_image, text_augmented_prompt)
...
def visual_revision_prompting(openai_client, input_image_file, original_output_image):
	...
	texts = extract_text_from_image(input_image_file)
	...
	html, prompt_tokens, completion_tokens, cost = gpt4o_revision_call(openai_client, input_image, original_output_image, prompt)
...
def layout_marker_prompting(openai_client, image_file, auto_insertion=False):
	...
	## call GPT-4V
	html, prompt_tokens, completion_tokens, cost = gpt4o_call(openai_client, orig_input_image, text_augmented_prompt)
...
"""
nlprun -m sphinx2 -g 0 -c 4 -r 40G -a sketch2code --output /sailhome/lansong/Sketch2Code/logs/d2c_github_gpt4o_direct_short_v0.txt 'python gpt4o.py --prompt_method direct_prompting --take_screenshot'

nlprun -m sphinx1 -g 0 -c 4 -r 40G -a sketch2code --output /sailhome/lansong/Sketch2Code/logs/d2c_gpt4o_revision_github_hard_v0.4 'python gpt4o.py --prompt_method revision_prompting --take_screenshot'
"""
...
if __name__ == "__main__":
	...
	test_data_dir = "/media/sda5/cyn-workspace/UI-SPEC/baselines/"
	cache_dir = "/media/sda5/cyn-workspace/UI-SPEC/baselines/cache/"
	...
	# 	predictions_dir = cache_dir + "gpt4o_layout_marker_prompting" + ("_auto_insertion" if args.auto_insertion else "") 
		...
		orig_data_dir =  os.path.join(cache_dir , "/media/sda5/cyn-workspace/UI-SPEC/baselines/cache/gpt4o_text_augmented_prompting")
